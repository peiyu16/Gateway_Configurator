...
const AWSPublish = () => {
    ...
    const gotoForm = () => {
        navigate('/mqttpublishform?source=aws');
    };

    const fetchTopics = async () => {
        setLoadingState(true);
        try {
            const response = await axios.get('http://raspberrypi.local:5000/api/awsiot/topics-publish');
    ...
    const getMqttStatus = async () => {
        try {
            ...
            setEnablePub(response.data.awsiotpublish);
            
        } catch (error) {
            console.error('Error fetching AWS IoT MQTT status:', error);
        }
    };
    ...
    const publishAction = async () => {
        ...
        try {
            ...
            if(enablePub){
                const response = await axios.post('http://raspberrypi.local:5000/api/awsiot/stop-all-publishing');
                console.log('Stop Publish:', response.data);
                alert(response.data.message);
                setEnablePub(false); // Set the state to false to stop publishing
            
            } else{
                const response = await axios.post('http://raspberrypi.local:5000/api/awsiot/topics-publish');
    ...
    const deleteAction = async (id) => {
        ...
        if (userConfirmed) {
            try {
                ...
                const response = await axios.delete(`http://raspberrypi.local:5000/api/awsiot/topics-publish/${id}`);
    ...
    const handleEditFormSubmit = async (data) => {
        ...
        setEditSuccess(null);
        try {
            const response = await axios.put(`http://raspberrypi.local:5000/api/awsiot/topics-publish/${currentTopic._id}`, data);
