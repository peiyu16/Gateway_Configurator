...
const IOEvent = () => {
    ...
    const fetchEvents = async () => {
        ...
        // This should be retriving IO events
        try {
            setloadingEvent(true);
            const response = await axios.get('http://raspberrypi.local:5000/api/ioevents');
    ...
    const handleEnable = async (event) => {
        try {
            ...
            const response = await axios.patch(`http://raspberrypi.local:5000/api/ioevents/${event._id}/enable`,
    ...
    const onSubmit = (data) => {
        if (isEditing) {
            setAction(true);
            // Edit event API call
            axios.put(`http://raspberrypi.local:5000/api/ioevents/${editingEventId}`, data)
                .then(response => {
                    fetchEvents();
                    handleClose();
                })
            ...
            setAction(true);
            // Add event API call
            axios.post('http://raspberrypi.local:5000/api/ioevents', data)
                .then(response => {
                    fetchEvents();
                    handleClose();
                })
    ...
    const handleDelete = (id) => {
        ...
        if (userConfirmed) {
            axios.delete(`http://raspberrypi.local:5000/api/ioevents/${id}`)
                .then(() => {
                    fetchEvents();
                })
                ...
                <Modal.Body>
                    <Form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <SelectInput 
                            ...
                            options={triggerpoint} // change to DI/AI
                        />
                        <SelectInput 
                            control={control} 
                        ...
                        />
                        <TextInput control={control} name="scanningCycle" label="Scanning Cycle (ms)" placeholder="Enter Scanning Cycle in ms" />
