...
const MQTTSubscribe = () => {
    ...
    const gotoForm = () => {
        navigate('/mqttsubform?source=mqtt');
    };
    ...
    const subscribeAction = async () => {
        ...
        try {
            ...
            if(enableSub){
                const response = await axios.post('http://raspberrypi.local:5000/api/mqtt/stop-all-subscribing');
                console.log('Stop Subscribing:', response.data);
                alert(response.data.message);
                setEnableSub(false); // Set the state to false to stop publishing
            } else {
              const response = await axios.post('http://raspberrypi.local:5000/api/mqtt/topics-subscribe');
    ...
    const deleteAction = async (id) => {
        ...
        if (userConfirmed) {
            try {
                ...
                const response = await axios.delete(`http://raspberrypi.local:5000/api/mqtt/topics-subscribe/${id}`);
    ...
    const fetchTopics = async () => {
        ...
        try {
            const response = await axios.get('http://raspberrypi.local:5000/api/mqtt/topics-subscribe');
    ...
    const getMqttStatus = async () => {
        try {
            ...
            setEnableSub(response.data.mqttsubscribe);
    ...
    const handleEditFormSubmit = async (data) => {
        ...
        setEditSuccess(null);
        try {
            const response = await axios.put(`http://raspberrypi.local:5000/api/mqtt/topics-subscribe/${currentTopic._id}`, data);
        ...
        <Container>
           ...
           <div className="d-flex justify-content-center" style={{marginTop:"50px"}}>
                ...
                { subscribing ?  (   
                    ...
                    enableSub ? "Subscribed" : "Subscribe"
                )} 
                </Button>
            </div>
