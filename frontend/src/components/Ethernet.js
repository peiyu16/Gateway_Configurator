...
const  Ethernet = () => {
    ...
    const fetchConfig = async () => {
          try {
              setloadingEth(true);
              const response = await axios.get('http://raspberrypi.local:5000/api/network/ethernet-config');
   ...
   useEffect(() => {
    ...
    const onSubmit = async (data) => {
      try {
          setAction(true);
          const response = await axios.post('http://raspberrypi.local:5000/api/network/ethernet-config', data);
